...
| 长时间不可预测任务 | 首版没有 owner lease renewal，`lockTtl` 难以选 |
...
`idempotent()` middleware 适合直接使用 `Pipeline`，或者未来 `defineModel()` 返回 `ctx.state.result` 的版本。当前 `@hile/model@2.1.1` 的 `defineModel()` 在缓存命中短路时无法把 `ctx.state.result` 作为最终返回值，因此不要把 `idempotent()` 当作当前 model 的一行式方案。
