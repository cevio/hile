...
```
@hile/message-modem       ← 传输无关的请求/响应协议
  ├── @hile/message-ws    ← WebSocket 实现
  ├── @hile/message-ipc   ← 进程间 IPC 实现
  └── @hile/message-worker-thread ← Worker 线程实现

@hile/message-loader      ← 文件系统消息路由加载

@hile/micro               ← 微服务框架（Server/Client/Registry/Application）
...
Registry (注册中心，9876 端口)
  ...
  └── Application B (namespace: "order-service")
      A 调用 B：app.call("order-service", "/-/createOrder", data)
```
