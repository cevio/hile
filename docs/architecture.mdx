...
flowchart TB
  ...
  subgraph core["基础层 Core Layer"]
    c1["@hile/core（Container · register/resolve/shutdown · 依赖追踪 · 循环检测 · 事件系统）"]
  end
...
**@hile/cache**：`defineCache()` 模板键语法定义缓存，`RedisCache(prefix, redis)` 构造时注入 ioredis 客户端，通过 `loadCache` 提供读穿透（read-through）的 `read` / `write` / `remove` / `has` 操作。
...
- `Application`：集成 Registry + 本地内存熔断器 + 重试 + 缓存降级 + topic 管理
