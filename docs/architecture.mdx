...
│  @hile/http (HTTP 服务)    @hile/http-next (Next.js 桥接)   │
│  业务代码 (controllers / services / models)                 │
├────────────────────────────────────────────────────────────┤
│                    集成层 (Integration Layer)                │
│  @hile/typeorm (数据库)   @hile/ioredis (Redis)             │
│  @hile/cache (类型安全缓存)                                  │
├────────────────────────────────────────────────────────────┤
│                    基础层 (Core Layer)                       │
│  @hile/core — Container 异步服务容器                        │
│  register / resolve / shutdown / 依赖追踪 / 事件系统          │
├────────────────────────────────────────────────────────────┤
│                    通信层 (Communication Layer)              │
│  @hile/message-modem (抽象协议)                             │
│    ├── @hile/message-ws (WebSocket)                        │
│    ├── @hile/message-ipc (IPC)                             │
│    └── @hile/message-worker-thread (Worker)                │
│  @hile/message-loader (消息路由)                             │
│  @hile/micro (微服务：Server/Client/Registry/Application)    │
