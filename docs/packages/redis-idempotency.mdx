...
`@hile/redis-idempotency` 提供一个 Redis-backed 幂等原语：同一个业务 key 和同一个 payload fingerprint 只允许一个 owner 执行业务函数，其他调用等待或复用第一次成功结果。
...
每个幂等键只占用一个 Redis key。value 是 JSON 字符串，状态只在 `IN_FLIGHT` 和 `DONE` 之间流转：
...
flowchart LR
    ...
    A -->|"ACQUIRE_OR_READ"| B
    B -->|"COMMIT_IF_OWNER<br/>owner token 匹配"| C
    B -->|"RELEASE_IF_OWNER<br/>fn 抛错"| A
    B -->|"TTL 过期"| D
...
- 避免 lock key 已过期但 result key 仍存在时的竞态窗口
- Lua 脚本可以原子地完成 acquire、read、commit、release
- `COMMIT_IF_OWNER` 会校验 owner token，旧 owner 不能覆盖新 owner 的状态
- `fingerprint` 和状态保存在同一个 value 中，payload mismatch 可以统一判断
...
<Note>
当前 `@hile/model@2.1.1` 的 `defineModel()` 还不是以 `ctx.state.result` 作为最终返回值，因此 `idempotent()` middleware 不适合直接挂在当前版本的 `defineModel({ pipelines })` 上做缓存命中短路。当前 model 推荐使用函数级 `withIdempotency()`。
</Note>
...
| 导出 | 类型 | 说明 |
|---|---|---|
| `withIdempotency(redis, key, fn, options)` | `function` | 函数级幂等保护，最常用 |
...
flowchart TD
    ...
    B["ACQUIRE_OR_READ Lua"]
    C{"脚本返回"}
    D["ACQUIRED: 执行 fn()"]
    E["fn 成功"]
    F["COMMIT_IF_OWNER Lua"]
    G["返回 result"]
    H["fn 失败"]
    I["RELEASE_IF_OWNER Lua"]
    J["抛原始错误"]
...
- 直接挂到 `@hile/model@2.1.1` 的 `defineModel({ pipelines })` 上做一行式缓存命中短路

原因是当前 `defineModel()` 返回内部局部变量 `result`，缓存命中短路时 `ctx.state.result` 不会成为 `model.handler()` 的返回值。
