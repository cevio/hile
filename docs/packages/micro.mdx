...
call() 被调用
    │
    ├── 获取活跃排除列表（30 秒冷却中）
    ├── app.get(namespace, exclude)
    │
    ├── 调用 client.request()
    │   ├── 成功 → recordSuccess()（从熔断名单移除）
    │   └── 失败 → recordFailure()（加入熔断名单，30 秒冷却）
    │              └── retries > 0 → 递归重试（--retries）
    │              └── retries = 0 → 抛出错误
    └── 如果所有实例都被排除 → 重置熔断器，重试
```
