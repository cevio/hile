---
title: "@hile/micro"
description: "WebSocket 微服务框架：服务注册与发现、心跳、熔断、重试、链路追踪、超时控制"
---

## 安装

```bash
pnpm add @hile/micro
```

---

## 概述

`@hile/micro` 是基于 `@hile/message-loader` 与 `@hile/message-ws` 的轻量级 **WebSocket 微服务框架**。提供服务注册与发现、心跳保活、熔断、请求超时、自动重试、调用链路追踪等功能。

### 架构分层

```
MessageLoader (路由) + MessageWs (请求/响应传输)
  └── Server（WebSocket 服务底层）
        ├── Registry（注册中心）
        └── Application（应用服务）
```

### 角色说明

| 组件 | 职责 |
|------|------|
| **Server** | WebSocket 监听、连接管理、消息路由。不关心注册中心 |
| **Client** | 远端 Server 的代理，提供 `request()` / `push()` 通信接口 |
| **Registry** | 注册中心。维护 namespace → 实例列表，心跳检测剔除死实例 |
| **Application** | 应用服务。集成注册发现、熔断、重试、追踪、超时等功能 |

一个 `Application` 实例 **同时** 扮演 provider（`register` 暴露接口）和 consumer（`get` / `call` 调用其它服务）。同一进程只需一个 Application 实例。

---

## 连接协议

### 出站 URL 格式

```
ws://{targetHost}:{targetPort}/{announceHost}/{listenPort}/{namespace}
```

- `announceHost`：构造时传入 `advertiseHost` 或自动获取的 IPv4
- `listenPort`：`listen(port)` 设置的端口
- `namespace`：构造时传入的 namespace 字符串

### 入站路径解析

Server 将 URL 路径按 `/` 分割为 `[callerHost, callerPort, ...extras]`。extras 以 `/` 分段，Registry 用 `extras.join('/')` 作为 namespace。

---

## 核心功能

### 快速开始

以下示例展示三个独立进程：Registry、服务 A（payments）、服务 B（checkout）。**每个服务进程只需要一个 `Application` 实例**，同时扮演 provider 和 consumer：

```typescript
import { Registry, Application } from '@hile/micro';

// 1. 启动 Registry
const registry = new Registry({ advertiseHost: '127.0.0.1' });
await registry.listen(9000);

// 2. 启动 Provider（服务 A）

const provider = new Application({
  namespace: 'payments',
  registry: { host: '127.0.0.1', port: 9000 },
  advertiseHost: '127.0.0.1',
});
await provider.listen(9100);
provider.register('/charge', async ({ data }) => {
  return { charged: true, amount: data.amount };
});

// 3. 启动 Consumer 调用（服务 B）

const consumer = new Application({
  namespace: 'checkout',
  registry: { host: '127.0.0.1', port: 9000 },
  advertiseHost: '127.0.0.1',
});
await consumer.listen(9200);

const result = await consumer.call('payments', '/charge', { amount: 100 });
console.log(result); // { charged: true, amount: 100 }
```

### 服务发现

按 namespace 从 Registry 获取远端 Client 并缓存：

```typescript
const client = await consumer.get('payments');
const result = await client.request('/charge', { amount: 100 });
```

- 首次查询通过 Registry `/-/find` 获取地址
- 结果**缓存**在内存中（namespace → Client），后续调用直接使用
- Client 断连时自动清理缓存，下次 `get()` 重新发现

### 熔断器

`call()` 自动跟踪每个 namespace 下各 peer 的调用失败。失败的 peer 被临时排除，**30 秒冷却期**后自动恢复。

| 场景 | 行为 |
|------|------|
| 调用 peer A 失败 | A 加入排除列表（30s cooldown） |
| 再次调用 | Registry `/‑/find` 带上 `exclude`，返回其他 peer |
| 所有 peer 都被排除 | 重置排除列表，从所有 peer 中重新选择 |
| 调用成功 | 该 peer 从排除列表中移除 |

### 请求超时

每个请求都有超时控制，超时后自动向对端发送 **ABORT** 消息取消远程执行：

```typescript
// 构造时设置全局默认超时
const app = new Application({
  namespace: 'svc',
  registry: { host, port },
  requestTimeoutMs: 10_000,  // 默认 30000ms
});

// 单次调用覆盖
await app.call('svc', '/api', data, { timeout: 5_000 });       // 5s 超时
await app.call('svc', '/api', data, { timeout: 1_000, retries: 0 }); // 1s 超时，不重试
```

### 手动取消请求

使用 `get()` 拿到 Client 后，`request()` 通过 `AbortSignal` 可主动取消正在等待响应的请求：

```typescript
const client = await consumer.get('payments');
const ac = new AbortController();

// 例如 5 秒后主动取消
setTimeout(() => ac.abort(), 5000);

try {
  const result = await client.request('/charge', { amount: 100 }, { signal: ac.signal });
} catch (err) {
  // 超时或手动 abort 都会 reject
}
```

- `abort()` 向对端发送 **ABORT 消息**，让远程 handler 提前终止
- 超时到期内部也是调用同一个 `controller.abort()`，机制相同
- 适用场景：用户取消、页面卸载、竞态淘汰

### 自动重试

`call()` 默认 retries=1，失败后自动换 peer 重试：

```typescript
await app.call('svc', '/api', data);                          // 默认重试 1 次
await app.call('svc', '/api', data, { timeout: 5000, retries: 3 });  // 超时 5s，重试 3 次
await app.call('svc', '/api', data, { timeout: 5000, retries: 0 });  // 超时 5s，不重试
```

重试策略：失败 → 该 peer 被排除 → 递归 `call(retries-1)` → 从排除列表中选其他 peer。

### Correlation ID 链路追踪

`call()` 自动为每次调用注入唯一 `_correlationId`：

```typescript
provider.register('/api', async ({ data }) => {
  console.log(data._correlationId); // 自动注入的 UUID
});
```

| 入参 data | 结果 |
|-----------|------|
| `null / undefined` | 包装为 `{ _correlationId, data: null }` |
| 字符串/数字 | 包装为 `{ _correlationId, data: '原始值' }` |
| 数组 | 包装为 `{ _correlationId, data: [原始数组] }` |
| `{ value: 1 }`（无 `_correlationId`） | 浅拷贝注入 `{ value: 1, _correlationId: 'uuid' }` |
| `{ _correlationId: 'trace-1' }` | 保留已有 ID，**不覆盖** |

**注意：** 原始 data 对象不会被修改（使用浅拷贝 `{ ...data, _correlationId }`）。

### 健康检查

每个 `Application` 实例自动注册 `/-/health` 端点：

```typescript
const health = await app.dispatch('/-/health', {});
// { status: 'ok', registry: true, uptime: 123.45, namespaces: ['payments'] }
```

| 字段 | 类型 | 说明 |
|------|------|------|
| `status` | `'ok'` | 固定值 |
| `registry` | `boolean` | 是否已连接 Registry |
| `uptime` | `number` | 进程启动时长（秒） |
| `namespaces` | `string[]` | 本地已缓存的 namespace 列表 |

### 缓存降级

当 Registry 不可用但本地仍有已缓存的 Client 连接时，`get()` 自动降级使用缓存：

| 场景 | 行为 |
|------|------|
| Registry 宕机 + 缓存有效 | 返回缓存 Client，继续服务 |
| Registry 宕机 + 缓存已过期 | 报错 |
| 全新 namespace + Registry 宕机 | 报错 |
| Registry 恢复 | 恢复正常查询 |

### Registry 心跳保活

- **Application** 每 10 秒向 Registry 推送 `/-/heartbeat`
- **Registry** 每 1 秒检查所有实例的心跳时间戳
- 超过 20 秒未收到心跳的实例被自动剔除并断开连接

### Registry 重连

Application 与 Registry 断开时自动重连，失败后 3 秒重试，直到 `listen()` 返回的 teardown 触发。

---

## 配置管理

### Registry 工作目录

Registry 启动时自动创建 `~/.registry/` 工作目录。YAML 配置文件存放在 `~/.registry/configs/` 下，按 namespace 分文件管理：

```
~/.registry/
  └── configs/
        ├── service-a.config.yaml
        ├── service-b.config.yaml
        └── global.config.yaml
```

### 配置文件热加载

Registry 监听 `configs/` 目录的文件变化，新增或修改 `*.config.yaml` 文件时自动加载（兼容 vim 原子写入），无需重启。

### 远程读取配置

通过 `/-/env/variables` 端点，已连服务可按 namespace 和字段从 Registry 远程读取配置：

```typescript
// Application 侧
const result = await app.getEnvVariables(
  { namespace: 'service-a', fields: ['db.host', 'db.port'] },
  { namespace: 'global' },
);
// result = {
//   'service-a': { 'db.host': '10.0.0.2', 'db.port': 3306 },
//   'global': { featureFlag: true },
// }
```

### CLI 管理配置

通过 `hile registry configs` 子命令可直接管理本机 `~/.registry/configs/` 下的 YAML 文件：

```bash
# 列出所有 namespace
hile registry configs

# 查看配置（YAML 输出）
hile registry configs get my-service

# 查看配置（JSON 输出）
hile registry configs get my-service --json

# 设置配置项
hile registry configs set my-service port=8080
hile registry configs set my-service debug=true

# 设置数组/对象（值以 { 或 [ 开头时自动 JSON 解析）
hile registry configs set my-service whitelist='["10.0.0.1","10.0.0.2"]'
hile registry configs set my-service db='{"host":"localhost","port":3306}'

# 删除整个 namespace（需确认）
hile registry configs del my-service

# 删除某个字段（需确认）
hile registry configs del my-service port

# 跳过确认删除
hile registry configs del my-service -y
```

运行中的 Registry 通过 `fs.watch` 自动感知文件变化，无需重启。

---

## API 参考

### ApplicationProps

```typescript
type ApplicationProps = {
  namespace: string;                           // 本服务 namespace
  registry: { host: string; port: number };    // Registry 地址
  registryLookupTimeoutMs?: number;            // /-/find 超时，默认 10000
  requestTimeoutMs?: number;                   // 单次请求超时，默认 30000
} & { advertiseHost?: string };                // 出站宣告地址
```

### Application

```typescript
class Application extends Server {
  constructor(props: ApplicationProps);

  // 启动监听，自动连接 Registry，启动心跳
  listen(port: number): Promise<() => Promise<void>>;

  // 获取 namespace 对应的远端 Client（缓存 + 自动发现）
  get(namespace: string, exclude?: string[]): Promise<Client>;

  // 一站式调用：get + request + response + 熔断 + 重试 + 追踪
  call<T = any>(
    namespace: string,
    url: string,
    data: any,
    options?: {
      timeout?: number,     // 请求超时（ms），默认 requestTimeoutMs
      retries?: number,     // 失败重试次数，默认 1
      signal?: AbortSignal, // 中止信号
    },
  ): Promise<T>;

  // 流式调用：获取远端 AsyncIterable 数据流
  stream(
    namespace: string,
    url: string,
    data: any,
    options?: {
      signal?: AbortSignal,
      retries?: number,
    },
  ): Promise<Readable>;

  // 注册路由（provider 侧）
  register<T = any>(url: string, handler: Function): () => void;

  // 同进程调用路由
  dispatch(url: string, data: any): Promise<any>;

  // 远程读取 Registry 的配置（强类型）
  getEnvVariables<
    T extends Record<string, Record<string, any>>,
    const Requests extends readonly EnvRequest<T>[],
  >(...data: Requests): Promise<GetEnvVariablesResult<T, Requests>>;
}
```

### Registry

```typescript
class Registry extends Server {
  constructor(props?: MicroServerProps);
  listen(port: number): Promise<() => Promise<void>>;
  onFind(): void;
  watchEnvFile(): fs.FSWatcher | undefined; // 监听 ~/.registry/configs/ 目录
}
```

### Client

```typescript
class Client extends MessageWs {
  request<T = any>(url: string, data: any, options?: { timeout?: number; signal?: AbortSignal }): Promise<T>;
  push(url: string, data: any, options?: { timeout?: number; signal?: AbortSignal }): void;
  dispose(): void;
}
```

---

## 与 @hile/core 的关系

`@hile/micro` **不依赖** `@hile/core`，可与任意 Node.js 进程独立使用，也可在 `defineService` 中接入容器。

---

## 相关包

- `@hile/message-loader` — 路由注册与 `dispatch`
- `@hile/message-ws` — WebSocket 上的请求/响应传输
- `ws` — 底层 WebSocket 实现
