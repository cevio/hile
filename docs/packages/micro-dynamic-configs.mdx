...
```
管理员/程序                      MicroDynamicConfigsServer          @hile/micro 主题              订阅方服务
    │                                    │                              │                          │
    │  save({ key: value })              │                              │                          │
    ├───────────────────────────────────►│                              │                          │
    │                                    │                              │                          │
    ...
    │                                    ├── emit('change:key')          │                          │
    │                                    │                              │                          │
    │                                    ├── publisher.update(value) ───►──── app.publish() ──────► subscribe 回调
    │                                    │                              │  topic: {ns}:{key}       │  收到新值
    │                                    │                              │                          │
```
...
Pass 1: 校验 & 对比
  │
  ├── 遍历 value 的每个 key
  ├── 检查 key 是否在 schema 中定义（未定义则跳过）
  ├── 用该字段的 Zod schema 校验新值（校验失败抛出错误）
  ├── 用 isDeepStrictEqual 与新值和旧值对比
  └── 记录有变更的条目
  │
Pass 2: 持久化 & 推送
  │
  ├── 构建全量新快照（Object.assign）
  ├── redis.set(redis_key, JSON.stringify(新快照))
  ├── 更新内存中的 _value
  ├── 对每个变更的字段：
  │   ├── publisher.update(newValue) — 推送到所有订阅者
  │   └── this.emit('change:' + key, newValue, oldValue) — 本地事件
  └── 返回变更数
```
