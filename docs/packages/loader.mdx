...
```
directory/
  ├── users.handler.ts         → /users
  ├── users/[id].handler.ts    → /users/[id]
  └── index.handler.ts         → / (defaultSuffix 处理)
         ↓
    scanDirectory(directory)
         ↓
    [
      { absolute: '/abs/users.handler.ts',  relative: 'users.handler.ts',  routePath: '/users' },
      { absolute: '/abs/users/[id].handler.ts', relative: 'users/[id].handler.ts', routePath: '/users/[id]' },
      { absolute: '/abs/index.handler.ts',   relative: 'index.handler.ts',  routePath: '/' },
    ]
```
