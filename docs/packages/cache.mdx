...
`@hile/cache` 是在 `@hile/ioredis` 之上构建的**类型安全读穿透缓存层**。它解决了以下问题：
...
```typescript
const cache = new RedisCache('myapp:')     // 所有 key 的前缀
...
```typescript
constructor(prefix: string): RedisCache
```
...
**注意**：`prefix` 不会自动添加分隔符，所以建议写成 `prefix: 'myapp:'`（包含冒号）。
...
const cache = new RedisCache('blog:')
...
const cache = new RedisCache('app:')
...
```typescript
// 可以创建多个 RedisCache 实例
const userCacheLayer = new RedisCache('user:')
const postCacheLayer = new RedisCache('post:')
const sessionCacheLayer = new RedisCache('session:')
```
...
const ops = await new RedisCache('').loadCache(cache)
...
<AccordionGroup>
  ...
  <Accordion title="缓存一直不命中（总是回源）">
    **检查清单**：
    ...
    - `@hile/ioredis` 默认服务是否已加载（检查 `auto_load_packages` 配置）
    - 确认 key 前缀和模板是否正确
  ...
  <Accordion title="如何为不同业务使用不同的 Redis 实例？">
    默认情况下 `@hile/cache` 使用 `@hile/ioredis` 的默认导出。如果需要不同实例，在 `@hile/ioredis` 层面自定义服务。`@hile/cache` 目前不支持切换 Redis 连接。
    ...
    **曲线解决方案**：
    ...
    // 手动创建并使用独立的 Redis 连接
    const myRedis = new Redis({ host: 'cache-specific-host' })
    // 然后直接使用 ioredis 的 SETEX/GET 等命令
    ```
  </Accordion>
