...
5. **开发痛点能力** — singleflight 合并并发 miss、negative cache 缓存空结果、stale cache 先返回旧值再后台刷新、tag 批量失效
...
| 选项 | 解决的问题 |
|---|---|
| `singleflight` | 同一个 key 并发 miss 时只允许一个调用回源，其它调用等锁释放后读缓存 |
| `stale: { ttl }` | fresh TTL 到期后继续短期保留旧值，读取旧值时后台刷新 |
| `negative: { ttl }` | `Cache(undefined)` 写入短 TTL 哨兵，避免不存在的数据反复回源 |
| `tags` | 将 key 记录到 tag set，使用 `removeTag(tag)` 批量失效 |
...
### defineCache<T, R>(key, fn, options?)
...
| `options` | `boolean \| DefineCacheOptions<T, R>` | 旧 boolean 表示 fieldable；对象形式配置 singleflight、stale、negative、tags |

<Note>`fieldable` 使用 Redis Hash 存储，不兼容 `stale` 和 `negative` 这类依赖字符串 payload 的策略；组合使用会在 `defineCache()` 阶段直接抛出配置错误。</Note>

**返回值**：`DefineCacheResult<T, R>` — `{ fn, key, fieldable, options }` 对象。
...
| `key` | `T` | Redis key 模板字符串 |
| `fieldable` | `boolean` | 是否使用 hash field 写入 |
| `options` | `DefineCacheOptions<T, R>` | 缓存策略选项 |
