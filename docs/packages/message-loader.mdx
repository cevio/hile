...
文件系统                        路由表                       分发
  ...
  ├── users/                   │                      │       找到路由 → 执行 fn
  │   ├── index.msg.ts ─────►  │ /users               │       未找到  → 抛出 NotFoundException
  │   └── [id].msg.ts  ─────►  │ /users/:id           │
  └── ...                      └──────────────────────┘
```
...
```
传输层（如何传数据）          路由层（如何路由请求）       业务逻辑
┌─────────────────┐        ┌──────────────────┐        ┌─────────────┐
│ @hile/message-ws│        │                  │        │ msg handler │
│ @hile/message-ipc│──────►│ @hile/message-   │───────►│ msg handler │
│ @hile/message-  │        │ loader           │        │ msg handler │
│ worker-thread   │        │                  │        └─────────────┘
└─────────────────┘        └──────────────────┘
      ^                           ^
      | 基于                       | 基于
