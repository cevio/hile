...
sequenceDiagram
    ...
    Parent->>Child: child = fork('./worker.js')
    Note over Parent: ipc = new MyIpc(child)
    Note over Child: ipc = new MyIpc()

    Parent->>Child: REQUEST: ipc.request({ action: 'compute', value: 42 })
    Note over Child: exec({ action: 'compute', value: 42 })
    Note over Child: → { result: 84 }
    Child->>Parent: RESPONSE: { result: 84 }
```
