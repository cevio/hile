...
```mermaid
stateDiagram-v2
    [*] --> Registered: defineService(key, fn)
    Registered --> Init: loadService() 首次调用

    state Init {
        [*] --> 工厂函数执行中
        工厂函数执行中 --> [*]: lifecycle='init', status=0\n发射 service:init
    }

    Init --> Ready: 工厂成功返回
    Init --> FailedStopping: 工厂抛错/reject

    state Ready {
        [*] --> 结果已缓存
        结果已缓存 --> [*]: lifecycle='ready', status=1\n发射 service:ready\n通知等待队列
    }

    Ready --> ShutdownStopping: container.shutdown()

    state FailedStopping {
        [*] --> 执行已有teardown
        执行已有teardown --> FailedStopped: lifecycle='stopping'\n发射 service:error
    }

    FailedStopped --> [*]: lifecycle='stopped'\n通知队列(reject)

    state ShutdownStopping {
        [*] --> LIFO执行所有teardown
        LIFO执行所有teardown --> ShutdownStopped: 发射 service:shutdown:start/done/error
    }

    ShutdownStopped --> [*]: lifecycle='stopped'
```
