...
defineService(key, fn)
  │  注册到容器（只登记，不执行）
  ▼
loadService(props) 首次调用
  │
  ▼
┌──────────────┐
│  init         │  lifecycle = 'init', status = 0
│  工厂函数执行  │  发射 service:init
└───┬──────┬───┘
    │      │
 成功│      │失败
    ▼      ▼
┌────────┐ ┌──────────┐
│ ready   │ │ stopping  │  lifecycle = 'stopping'
│status=1 │ │ 执行已有  │  发射 service:error
│缓存结果 │ │ teardown  │
│通知队列 │ └────┬─────┘
└───┬────┘      ▼
    │      ┌──────────┐
...
container.shutdown()
    ...
    ▼
┌──────────┐
│ stopping  │  按 LIFO 顺序执行所有 teardown
│(每个服务)│  发射 service:shutdown:start/done/error
└────┬─────┘
     ▼
┌──────────┐
