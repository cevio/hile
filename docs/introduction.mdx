...
<CardGroup cols={2}>
  ...
  <Card title="Next.js 全栈集成" icon="globe">
  ...
  <Card title="Redis 集成" icon="zap">
    ioredis 客户端服务化封装。提供 <code>createRedis()</code> 手动模式和容器模式。自动连接等待就绪、自动断连、环境变量配置。一行 <code>auto_load_packages</code> 即可在容器中可用。
  </Card>
  <Card title="类型安全缓存" icon="database-zap">
    基于 <code>@hile/ioredis</code> 的读穿透缓存层。模板字符串键类型推导——<code>defineCache('user:{id:string}:profile', fn)</code> 自动推导参数类型。提供 <code>read</code>（读穿透）、<code>write</code>（强制回源）、<code>remove</code>（删除）、<code>has</code>（存在检查）四种操作，支持 TTL 过期。
  </Card>
  <Card title="消息通信体系" icon="message-circle">
  ...
  <Card title="轻量级服务发现" icon="network">
  ...
  <Card title="运行时动态配置" icon="sliders-horizontal">
  ...
  <Card title="结构化日志" icon="scroll-text">
  ...
  <Card title="文件加载器抽象" icon="folder-tree">
...
| `@hile/cache` | [![npm](https://img.shields.io/npm/v/@hile/cache?style=flat-square)](https://www.npmjs.com/package/@hile/cache) | **类型安全 Redis 缓存**——基于 `@hile/ioredis`。`defineCache` 模板键语法支持 `{name:string|number|boolean}` 占位符，自动推导参数类型。`RedisCache` 类通过 `loadCache` 返回 `{read, write, remove, has}` 四种操作，内部 `loadService(ioredisService)` 获取 Redis。 | `pnpm add @hile/cache` |
...
```
             ...
             ┌─────────────────────────────────────────────────┐
             │                  @hile/core                     │
             │    Container: register / resolve / shutdown     │
             │    依赖追踪 · 循环检测 · 事件系统 · 超时控制      │
             └──┬───────┬────────┬────────┬───────────────────┘
                │       │        │        │
    ┌───────────▼─┐ ┌──▼──────┐ │  ┌─────▼──────────┐
    │ @hile/cli   │ │@hile/   │ │  │ @hile/typeorm  │
    │ hile start  │ │logger   │ │  │ createDataSource│
    │ hile registry│ │create   │ │  │ transaction()  │
    └─────┬───────┘ │Logger() │ │  └────────────────┘
          │         └─────────┘ │
          │                     │  ┌─────────────────┐
          │ 依赖 micro          │  │  @hile/model    │
          │ (registry 命令)     │  │ defineModel     │
          │                     │  │ loadModel       │
          │                     │  │ Pipeline        │
          │                     │  └─────────────────┘
          │         ┌───────────▼──────────┐
          ...
          │   ┌──────────────────────────────────────────────────┐
          │   │               @hile/loader                       │
          │   │   Loader 抽象基类 · scanDirectory                │
          │   │   compileRoutePath · toRouterPath · normalizePath│
          │   └──┬───────────────────┬────────────────┬─────────┘
          │      │                   │                │
          │      ▼                   ▼                ▼
          │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐
          │  │ @hile/http   │ │@hile/message-│ │@hile/schedule│
          │  │ Http 类      │ │loader        │ │Scheduler     │
          │  │ define       │ │MessageLoader │ │defineJob     │
          │  │ Controller   │ │defineMessage │ │              │
          │  └──────┬───────┘ └──────┬───────┘ └──────────────┘
          │         │                │
          │         ▼                ▼
          │  ┌──────────────┐ ┌─────────────────────────────────┐
          │  │@hile/http-   │ │        @hile/micro              │
          │  │next          │ │  Server → Registry              │
          │  │HttpNext 类   │ │         → Application           │
          │  └──────────────┘ │  Client（继承 MessageWs）        │
          │                   └───────────┬─────────────────────┘
          │                               │
          │                   ┌───────────▼─────────────────────┐
          ...
          ▼
┌──────────────────────────────────────────────────────────────┐
│              消息通信传输层（独立于 @hile/core）                 │
│                                                              │
│  @hile/message-modem  (抽象基类，零外部依赖)                    │
│     ├── @hile/message-ws               (WebSocket, 依赖 ws)    │
│     ├── @hile/message-ipc              (IPC, 零外部依赖)        │
│     └── @hile/message-worker-thread    (Worker, 零外部依赖)     │
└──────────────────────────────────────────────────────────────┘
```
...
<CardGroup cols={3}>
  ...
  <Card title="服务容器" icon="box" href="/fundamentals/container">
  ...
  <Card title="HttpNext 全栈" icon="globe" href="/fundamentals/http-next-pipeline">
  ...
  <Card title="缓存策略" icon="database-zap" href="/techniques/caching">
    Redis · 类型安全缓存 · 读穿透 · Cache-Aside 模式
  </Card>
  <Card title="消息通信" icon="message-circle" href="/techniques/messaging">
    MessageModem · 三种传输 · MessageLoader · 微服务框架
  </Card>
  <Card title="配置管理" icon="settings" href="/techniques/configuration">
