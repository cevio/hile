...
| `@hile/rsc-next` | Host only | Decode Flight and adapt Next-version-specific document delivery | Plugin compilation or domain behavior |
...
- `@hile/rsc-next` owns the Next-version-specific Flight decoder and optional content-first document delivery adapter. It is the only RSC package allowed to depend on Next-private wire details.
...
`RscPluginService.prepare()` imports and validates the active renderer without executing route components. The official Hile plugin runtime waits for this readiness barrier before attaching transport, listening, or publishing discovery, bounded by `prepareTimeoutMs` (30 seconds by default), so a published instance can serve its first render without paying module and Flight-runtime initialization cost. Custom renderers without a preparation hook remain compatible and are treated as immediately ready.

`RscPluginService.activate()` prepares an official replacement before atomically switching new render requests to its verified manifest/renderer pair, while in-flight requests finish on their captured revision. `RscPluginService.load()` atomically replaces the model snapshot; `bindRscModelDevelopment()` can watch model files without rebuilding RSC assets. The plugin updates its discovery announcement only after activation; `HileRscDiscoveryHost` authorizes, downloads, and enables that revision, then the Host may compose `createRscDevelopmentEventMiddleware()` and `RscDevelopmentReload` for browser refresh.
...
- `@hile/rsc-next` is the isolated Next-private adapter. Keep every Next-version-specific import and inline Flight wire rule there.
...
- The compiler records non-negative artifact byte sizes, plugin-wide versus client-scoped styles, per-client style reachability, and route-scoped client-reference graphs. Routes default to safe asset preload, may opt out with `prefetch: "none"`, and may explicitly permit full Flight prefetch with `prefetch: "route"`.
- Manifest server-function, client, chunk, style, route, and per-route reference collections have explicit protocol limits so one valid-looking artifact cannot create unbounded catalog or preload fan-out.
- The public asset middleware exposes only browser modules, browser chunks, styles, route preload metadata, and a sanitized browser manifest; server and SSR artifacts remain internal.
...
- A document-heavy Host may explicitly register `createContentFirstRscHtmlMiddleware()` before `HttpNext.start()`. The adapter streams ordinary SSR HTML while retaining only a bounded number of byte-identical inline `self.__next_f` scripts and emits them in their original order before `</body>`. When the bound is exceeded, it flushes retained scripts at that point and passes all remaining bytes through unchanged. At the first body write, only the exact `text/html` media type with no content encoding or identity encoding is transformed; normal Next RSC, asset, and Server Function responses therefore bypass it. The bound applies per in-flight HTML response and must be sized for concurrency. The trade-off is later hydration in exchange for earlier complete document text; Host and third-party scripts must not depend on observing Next's private `self.__next_f` state before the body completes. It is not the default.
