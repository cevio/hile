...
class McpProviderHost {
  ...
  constructor(private readonly application: HileMcpProviderApplication) {
    this.unregister = application.register(MCP_OPERATIONS.invoke, async ({ data, signal }) => {
      ...
      return streamExecution(base, invocation => Promise.resolve(invokeCapability(entry.provider, trustedRequest, invocation)));
    });
  }
...
class McpProviderAttachmentRuntime implements McpProviderAttachment {
  ...
  private async performClose() {
    ...
    if (this.unregisterPending) {
      try { this.unregister(); this.unregisterPending = false; } catch (error) { errors.push(error); }
    }
...
export async function attachMcpProvider(
  ...
  try {
    ...
    unregister = acquireProviderHost(application).add(instanceId, provider, manifest.fingerprint, options.invocationSecurity);
    topic = `${MCP_PROVIDER_TOPIC_PREFIX}${provider.id}/${instanceId}`;
    publication = await application.publish(topic, manifest);
    return new McpProviderAttachmentRuntime(provider, manifest, () => publication!.unpublish(), unregister, unload);
  } catch (cause) {
    const errors = [cause];
    ...
    if (unregister) try { unregister(); } catch (error) { errors.push(error); }
