...
`HileRscPluginRuntime` composes one plugin microservice lifecycle: prepare the active renderer, attach RSC transport operations, start the internal listener, publish the artifact with explicit discovery security, bind optional development activation, and close every phase in retryable order. Preparation imports and validates the official immutable renderer without executing a route component. A preparation failure or the bounded `prepareTimeoutMs` (30 seconds by default) prevents both listening and Registry publication, so discoverability is a readiness guarantee rather than merely process liveness. Plugin packages only provide configuration and optional adapters; no public HTTP listener is created.
