...
Build configuration metadata and build-scoped `styles` are copied into every immutable development revision. Relative shared styles inside the project source tree participate in source watching; package-export or absolute styles are refreshed by a manual rebuild or config reload. A metadata config reload creates a new revision, so Host navigation never observes presentation data from a different build than the rendered plugin.
