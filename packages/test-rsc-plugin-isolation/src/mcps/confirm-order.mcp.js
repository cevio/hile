...
export default defineMcpTool({
    ...
    const confirmation = acceptedContent(context.inputResponses, 'confirmation');
    if (!confirmation)
        return inputRequired({ inputRequests: { confirmation: inputRequired.elicit({
                    message: `Confirm demo order ${order_id}?`, requestedSchema: z.object({ confirmed: z.boolean() }),
                }) } });
    if (!confirmation.confirmed)
        return { isError: true, content: [{ type: 'text', text: `Order ${order_id} was not confirmed.` }] };
